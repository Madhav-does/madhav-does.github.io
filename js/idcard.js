  ...
  createFrontTexture() {
    ...
    const drawCard = (photoImg = null) => {
      ...
      if (photoImg && photoImg.complete && photoImg.naturalWidth > 0) {
        // Draw real profile photo inside circular frame with glowing cyber border
        const photoR = 64;
        const photoCenterX = avX + avW / 2;
        const photoCenterY = avY + 80;

        ctx.save();
        ctx.beginPath();
        ctx.arc(photoCenterX, photoCenterY, photoR, 0, Math.PI * 2);
        ctx.clip();
        ctx.drawImage(photoImg, photoCenterX - photoR, photoCenterY - photoR, photoR * 2, photoR * 2);
        ctx.restore();

        // High-tech glowing cyan circle border around photo
        ctx.save();
        ctx.strokeStyle = "#38bdf8";
        ctx.lineWidth = 3;
        ...
        // Name & Verification below photo
        ctx.fillStyle = "#ffffff";
        ctx.font = "bold 13px 'Inter', sans-serif";
        ctx.textAlign = "center";
        ctx.fillText("MADHAV RAJ M", avX + avW / 2, avY + avH - 28);

        ctx.fillStyle = "#10b981";
        ctx.font = "bold 11px 'Inter', sans-serif";
        ctx.fillText("● ID VERIFIED", avX + avW / 2, avY + avH - 10);
      } else {
        // Stylized Cyber Monogram / Fallback Profile Graphic
